...
export default function SecondPage() {
    ...
    const memoizedLogo2023 = useMemo(() => {
        return (
            <div className={styles.partieGauche}>
                <img src={logo} className={styles.logoCommon}></img>
                <div className={styles.rectangleAnnee}>
                    <div className={styles.texte1}>2023</div>
                    <div className={styles.texte2}>Résumé</div>
                </div>
            </div>)
    }, []);

    const optionsOD = generateItineraryList(getDistinctOD());
    const [chartData, setChartData] = useState(intemperiesClimatiquesList);
    const [activeButtons, setActiveButtons] = useState(["tous"]);
    const [value, setValue] = useState(optionsOD[1]);
    const [inputValue, setInputValue] = useState('');

    const formattedChartData = getChartDataFilteredByIntemperieNew(chartData);

    // Get all incidents record of axe Sud Est > OD
    function getDataFilteredAxeSudEstByOD() {
        if (!!value && value !== "") {
    ...
    */
    ...
    // customized theme for Mui Autocomplete
    ...
    function getChartDataFilteredByIntemperieNew(intemperieList) {
        ...
        const dataFiltered = getDataFilteredAxeSudEstByOD();
        try {
            intemperieList.forEach(intemperie => {
                if (!!dataset1 && Array.isArray(dataset1) && Array.isArray(dataFiltered) && !!dataFiltered) {
                    getDataFilteredAxeSudEstByOD()
...
// get one itinerary selected for each two-way trip
    function generateItineraryList(originalList) {
        ...
        originalList.forEach(route => {
            ...
            const reversedStationsSecondTrain = reversedStations[1] + " BV";
            const reversedRoute = reversedStations[0] + " - " + reversedStationsSecondTrain;
    ...
    function modifyActiveButtonsListWhenClickTous() {
        if (activeButtons === undefined) {
            ...
            setChartData(intemperiesClimatiquesList);
        } else {
            let modifiedList;
            ...
            if (!!data && Array.isArray(data)) {
                return (
                    <div className="custom-tooltip" style={{
                        ...
                        <p className="desc">{`Minutes perdues : ${data[3]}`}</p>
                    </div>
                );
            }
        }
    ...
    return (
        <>
            ...
            <div className={styles.partieBasse}>
                <div className={styles.sectionButtonsAndChart}>
                    <div className={styles.sectionButtonsAndODSelector}>
                        <div className={styles.sectionButtons}>
                            <div className={styles.firstIntemperieButtonSection}>
                                ...
                                <IntemperieButton oneVector={false} name={"Vent fort"} percentage={"20%"}
                                                  cssStyleName={"ventFort"}
                            ...
                            <div className={styles.secondIntemperieButtonSection}>
                                ...
                                <IntemperieButton oneVector={false} name={"Orage/Grêle"} percentage={"20%"}
                                                  cssStyleName={"orage"}
                                ...
                                <IntemperieButton oneVector={false} name={"Neige"} percentage={"8%"}
                                                  cssStyleName={"neige"}
                        ...
                        >
                            ...
                            <Tooltip content={<CustomTooltip/>}/>
                            {
                                intemperiesClimatiquesList.map(e => (
                                    <Line key={e} type="monotone" dataKey={e} stroke={getSchemeColor(e)}
                                          activeDot={{r: 6}}/>))
                            }
                        </LineChart>
                    </ResponsiveContainer>
